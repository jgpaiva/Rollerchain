...
if [ $# -ne 7 ]
then
  echo "NOT TO BE USED WITHOUT sendAndRunSingle!"
  exit 65
fi
...
ssh $HOSTNAME "mkdir -p $ROLLERCHAINPATH/$MYFOLDER/"
scp $JARFILE $CONFIGFILE $HOSTNAME:$ROLLERCHAINPATH/$MYFOLDER
ssh $HOSTNAME "cd $ROLLERCHAINPATH/$MYFOLDER; java -cp "$JARFILEB:$HOMEFOLDER/$LIB1:$HOMEFOLDER/$LIB2" $JAVACLASS $CURRENT $MASTER 1>out.out 2>out.err &"
